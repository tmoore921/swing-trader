...
- `risk_engine.py` — 1% risk sizing, fractional shares, R:R. **Position size is clamped** so notional never exceeds `min(MAX_POSITION_PCT × account, cash_available)` — a tight stop can otherwise demand more shares than the whole account holds. `atr()` / `atr_stop()` provide ATR(14)-based stops. `swing_highs()` finds fractal pivot highs and `estimate_targets()` sets T1 to the nearest such pivot at least `MIN_RESISTANCE_DISTANCE_PCT` above entry, falling back to a `MEASURED_MOVE_R` measured move when a breakout has no overhead supply left. **Do not revert T1 to "lowest daily high above entry"** — that was the 2026-09 bug that produced R:R of ~0.2 and blocked every order for a month (see `prompts/README.md`).
...
- `evaluate.py` — orchestrates the above into one `agent_action` per ticker. Stop = the wider of the structural SMA50 stop (fallback 8%) and an ATR-based stop, so a stop hugging the entry can't oversize or whipsaw. **Buy-zone band:** because the entry is a *limit buy at the pivot*, an order is queued while price is anywhere from `BUY_ZONE_BELOW_PIVOT_PCT` below the pivot (the order waits for the breakout) up to `BUY_SIGNAL_THRESHOLD_PCT` above it; beyond that is `extended` (don't chase) and below it is "still building base" (watchlist). This band was widened from the old razor-thin 0..+2% window, which was the reason almost nothing ever fired. Inside the band a `PLACE_ORDER` also requires R:R ≥ `MIN_RR` plus (toggleable via `REQUIRE_VOLUME_CONFIRMATION` / `REQUIRE_NEAR_52W_HIGH`) volume confirmation and price within `NEAR_52W_HIGH_PCT` of the 52-week high — otherwise it demotes to the watchlist. The stop is additionally floored at `MAX_STOP_DISTANCE_PCT` below entry so an unbounded ATR/SMA stop can't crush R:R. As of 2026-09 `REQUIRE_VOLUME_CONFIRMATION` is **off** by default (volume is reported, not vetoed). A liquidity floor (`MIN_PRICE`, `MIN_AVG_DOLLAR_VOLUME`) skips thin names. Each valid bullish setup also gets an `options_play` blueprint (see `options.py`). Fundamentals are deliberately NOT checked here (Twelve Data free tier is price-only) — `fundamentals_unverified: true` signals the agent to verify before buying.
- `options.py` — builds a deterministic **long-call blueprint** for each valid bullish setup: expiration window (`OPTIONS_MIN/TARGET/MAX_DTE`, ~30–60 DTE), target strike (the breakout pivot) and `OPTIONS_TARGET_DELTA` (~0.65, slightly ITM), a premium budget capped at `OPTIONS_MAX_PREMIUM_PCT` of the account / available cash, a liquidity gate, and an exit tied to the *underlying's* stop (not expiry). It cannot price contracts (the free data tier has no chains/greeks) — it emits structure; the agent fetches the live chain and executes. Set `OPTIONS_ENABLED=False` to drop options from the output. Surfaced in the JSON as `instructions_for_agent.options_candidates` and as `options_alternative` on each share order.
...
- **Feed account state back in.** Pass `--cash` (from `get_accounts`) and `--existing-risk` (Σ open risk from `get_equity_positions`) so sizing and the heat cap are real, not blind. `--existing-risk` should also include open **option premium** (a long call's max loss), since options now count against the same 6% heat cap.
- **Options execution.** For each `options_candidates` / `options_alternative` blueprint the prompt chooses to act on, it fetches the live chain (`get_option_chains` / `get_option_quotes`), picks the call nearest the target delta/strike inside the DTE window, sizes within the premium budget, and places it (`review_option_order` → `place_option_order`). Per ticker it takes the shares **or** the call, never both, and manages the call by the underlying's stop/targets — not by expiry.
